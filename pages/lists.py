...
mode = st.radio("操作を選択", ["新規リスト作成", "既存リストを編集"], horizontal=True)
st.divider()
...
if mode == "新規リスト作成":
    ...
    if st.button("作成する", type="primary"):
        if new_name.strip():
            ...
            st.success(f"「{new_name.strip()}」を作成しました。「既存リストを編集」から候補者を追加できます。")
        else:
            st.warning("リスト名を入力してください")

else:
    if not lists:
        st.info("まだリストが作成されていません。「新規リスト作成」から作成してください。")
        st.stop()
